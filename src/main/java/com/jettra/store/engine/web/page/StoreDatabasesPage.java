 ...
 * Features: - Unified JettraFlux panel integrating Multi-Model Components
 * overview and Authorized Active Databases - Fine-grained access control and
 * permission filtering (READ / ADMIN) based on active SecurityContext - Native
 * JettraConfirmDialog for destructive database drop confirmations - Full
 * support for Java 25 RECORDS engine and 9 multi-model storage backends -
    ...
    public synchronized int syncDatabaseUsers(String targetDb, Set<String> selectedUsernames) {
        ...
        if (userRepo != null) {
            for (JUser ju : userRepo.findAll()) {
                ...
                if (uname == null || "admin".equalsIgnoreCase(uname)) {
                    continue;
                }
                if (allUsers.stream().anyMatch(u -> u.username().equalsIgnoreCase(uname))) {
                    continue;
                }
...
//                ).modifier(new Modifier().cssClass("btn-action btn-secondary").style("padding:6px 12px; font-size:12px;")));
    ...
    /**
     * Evaluates whether the current security principal possesses access
     * permissions (READ or ADMIN) over a specific database namespace.
     */
